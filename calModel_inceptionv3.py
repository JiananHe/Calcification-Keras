...
def InceptionV3_model(input_shape):
    base_model = InceptionV3(weights='imagenet', include_top=False, pooling=None, input_shape=input_shape, classes=2)
...
if __name__ == "__main__":
    ...
    # load model
    model = InceptionV3_model(input_shape)
    model.summary()
    
    # train
    history = model.fit_generator(train_generator, steps_per_epoch=40, epochs=150,
                                     validation_data=valid_generator, validation_steps=20)
    
    model.save("inception-cal.h5")
